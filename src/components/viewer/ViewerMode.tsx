          ...
          >
            ...
            {currentScreen.hotspots.map((hotspot) => (
              <motion.div
                ...
                onClick={(e) => hotspot.action.type !== 'map' && handleHotspotClick(hotspot, e)}
                className={`absolute ${hotspot.action.type !== 'map' ? 'cursor-pointer' : ''} group`}
                style={{
                  left: `${hotspot.x}%`,
                  top: `${hotspot.y}%`,
                  width: `${hotspot.width}%`,
              ...
              >
                {/* 지도 타입일 때는 실제 지도 표시 */}
                {hotspot.action.type === 'map' ? (
                  <MapHotspot
                    startAddress={hotspot.action.mapStartAddress}
                    endAddress={hotspot.action.mapEndAddress}
                  />
                ) : (
...
const MapHotspot = ({ startAddress, endAddress }: MapHotspotProps) => {
    ...
    <div className="w-full h-full rounded-lg overflow-hidden">
      ...
      >
        ...
        {startLocation && (
          ...
          >
            <div style={{ padding: '5px', color: '#000', fontSize: '10px', whiteSpace: 'nowrap' }}>
              출발: {startAddress}
            </div>
          </MapMarker>
        )}
